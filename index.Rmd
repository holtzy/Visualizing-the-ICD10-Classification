...
```{r}
devtools::install_github("holtzy/circlepackeR")

library(circlepackeR)         # devtools::install_github("jeromefroe/circlepackeR")


# select column in the ICD 10 classif
ICD_10 = ICD %>%  select(short, meaning_L2, meaning_L3) %>% unique() %>% droplevels() %>% mutate(value=1) %>% na.omit()
...
# Make the plot
widg=circlepackeR(temp, size = "value", color_min="white", height=600, width=600)
#library(htmlwidgets)
saveWidget(widg, file = "~/Desktop/test_circle_pack.html", selfcontained = TRUE)
...
# Create an edge list
edges_level1_2 = ICD %>% select(meaning_L1, meaning_L2) %>% unique %>% rename(from=meaning_L1, to=meaning_L2)
edges_level2_3 = ICD %>% select(meaning_L2, meaning_L3) %>% unique %>% rename(from=meaning_L2, to=meaning_L3)
edges_level3_4 = ICD %>% select(meaning_L3, meaning_L4) %>% unique %>% rename(from=meaning_L3, to=meaning_L4)
