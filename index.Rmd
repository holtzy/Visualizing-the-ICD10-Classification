...
> The [World Health Organization](https://en.wikipedia.org/wiki/World_Health_Organization) proposes a way to classify human diseases: the [ICD10 Classification](https://en.wikipedia.org/wiki/ICD-10). About 100k diseases are defined and organized. This page provides a few way to visualize this information.


# Choose your flavor {.tabset .tabset-fade .tabset-pills}
***
...
The ICD10 classification has been downloaded on the WHO website as a text file in September 2017. The first step of this work is to modify the format to get something more confortable to work with. The result clean file is available here in case you would like to conduct further analysis.  
...
The ICD10 classification is describe in this diagram:  
...
## Table
This table displays all the diseases stored in the ICD10 classification. Use the search bar to find a disease you are interested in.  
...
tmp = ICD %>% 
  ...
  select(Chapt, short, code_L2, meaning_L2, coding_L3, meaning_L3, coding_L4, meaning_L4) 

datatable(tmp, filter = 'top')
```
...
collapsibleTree(ICD_10, c("short", "meaning_L2", "meaning_L3"), width="2000px", height="800px", fontSize = 12)
```
...
# basic treemap
d3tree2(treemap(ICD_10,
            index=c("short", "meaning_L2","meaning_L3"),
            vSize="value",
            type="index"
            ) ,  rootname = "The 22 groups of the ICD10 classification", width = "100%", height="700px" ) 
```
